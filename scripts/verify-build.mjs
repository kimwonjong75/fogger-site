...
/* ---------- 배송 조건 표기 ----------
 *
 * 2026-08-10: 부품 사양 문구를 검사하던 "광고 규칙"(타사 언급·화재·성능 우위·후기 인용)은
 * 사장님 지시로 삭제했다. 어떤 문구를 쓸지는 이제 사장님이 정한다.
 *
 * 배송 조건만 남긴 이유는 성격이 다르기 때문이다 — 이것은 "무슨 말을 쓰면 안 되는가"가
 * 아니라 **빠뜨리면 안 되는 고지가 비었는가**를 보는 검사다. "무료배송"만 단독으로 쓰면
 * 도서·산간 추가비와 반품 배송비를 고지하지 않은 것이 되어 주문 뒤에 분쟁이 된다.
 * 이것도 필요 없으면 이 블록을 통째로 지우고 config.yml 의 안내문도 함께 지운다.
 */
{
