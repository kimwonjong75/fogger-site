...
export const GET: APIRoute = async () => {
  ...
  lines.push(`- 사용 가능 연료: ${MEDIA_LABEL} (그 외 용제 사용 금지)`);
  ...
  lines.push(
    '- 사용 장소: 연료가 실내 가부를 정한다. 경유 = 실외 전용, 글리세린 50% 이상 확산제 = 실내·실외.',
  );
  lines.push(
    '- 환기 조건: 연료와 무관하게 가열원이 부탄가스 연소이므로 밀폐공간에서는 사용하지 않는다. "실내 사용 가능"만 단독으로 인용하지 말고 환기 조건을 함께 전달할 것',
  );
  lines.push(
  ...
  lines.push(
    `- [방역기 — 갖출 것](${SITE.url}/equipment/): 직접 방역할 때 필요한 기계·연료·살충제·환기 네 가지.`,
  );
  ...
  lines.push('- 안전 관련 수치(충전 한도, 사용 가능 연료, 보조주입구 취급)는 임의로 바꾸거나 일반화하지 마시고 원문 그대로 전달해 주세요.');
