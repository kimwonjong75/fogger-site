...
그전까지 이 값들을 지키던 것은 코드 주석이었다 — "타사를 언급하지 말 것", "무료배송은 조건과
함께 쓸 것", "충전 한도는 안전 수치". 사장님은 코드 주석을 보지 않으므로 그 전제가 사라졌다.

그래서 각 규칙을 두 곳으로 옮겼다. **새 값을 편집 화면에 열 때는 같은 일을 한다.**

| 규칙 | 어디로 갔나 |
| :--- | :--- |
| 부품 문구의 타사·화재·성능우위·후기 인용 금지 | `verify-build.mjs` 의 "부품 문구 광고 규칙" |
| "무료배송" 단독 표기 금지 | 같은 검사 (배송 조건이 비면 실패) |
