...
**규칙은 주석이 아니라 검증기에 둔다.** 기계 검사는 `verify-build.mjs`의 "검색 전제 (AEO·SEO)"
블록에 있다: 제목 완전 중복(실패), 제목 단어의 본문 뒷받침(경고), 1,500mL 오독 방지 문장(실패),
ProductGroup·FAQPage 존재(실패), llms.txt의 오독 방지 줄(실패).
